...
//step: generate a set of available_tags (we can't pick a region because that will cause a weighting problem)
//step: pick a tag from available tags
...
function generate_question_ui(sentences) {
    ...
    //sets up quick_click on word selector
    if (game_mode === Mode.quick) {
        wordSel.click_callback = quick_click;
    }

    //highlight needed word
    if (game_mode === Mode.drop) {
...
function quick_click (word_selector, index) {
    ...
    //todo did the below get broken somewhere along the line
    submit_answer();
