var closest = function (list, value) {
    var min_less = Math.min(...list.map(function (x) {return value - x}).filter(function (x) {return x >= 0}));
    var min_more = Math.min(...list.map(function (x) {return x - value}).filter(function (x) {return x >= 0}));
