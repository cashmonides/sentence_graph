...
////// PURELY FUNCTIONAL UTILS ///////

//// purely functional list utils

//// purely functional dictionary utils

var create_map_with_one_value = function (list_items, value) {
	var h = {};
	for (var i = 0; i < list_items.length; i++) {
	    h[list_items[i]] = value;
	}
	return h;
}

var create_map_with_values_determined_by_function = function (list_items, f) {
	var h = {};
	for (var i = 0; i < list_items.length; i++) {
	    h[list_items[i]] = f(list_items[i]);
	}
	return h;
}

// f takes a key and value and returns a boolean.
var filter_dictionary = function (unfiltered_dict, f) {
	var filtered_dict = {};
	for (var key in unfiltered_dict) {
	    var value = unfiltered_dict[key];
	    // if the key and the value satisfy our criteria...
	    if (f(key, value)) {
...
var alter_all_dictionary_values = function (unaltered_dict, f) {
	var altered_dict = {};
	for (var key in unaltered_dict) {
	    var value = unaltered_dict[key];
	    altered_dict[key] = f(value);
	}
	return altered_dict;
}
