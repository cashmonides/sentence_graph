...
var create_execute = function () {
	...
	var _q = function (a, b) {
		test_op_args_funcs(a, b, 'question mark');
	    return function (c) {
	    	test_func_args_funcs(c, 'curried question mark');
	        return function (x) {
	            return a(x) ? b(x) : c(x);
	        }
	    }
	...
	var test_op_args_funcs = function (a, b, name) {
		if (typeof a !== 'function') {
			throw 'first argument to "' + name +
			'" operator is not a function';
		} else if (typeof b !== 'function') {
			throw 'second argument to "' + name +
			'" operator is not a function';
		}
	}
	
	var test_func_args_funcs = function (a, name) {
		if (typeof a !== 'function') {
			throw 'argument to "' + name +
			'" function is not a function';
		}
	}

	var _dot = function (a, b) {
		test_op_args_funcs(a, b, 'dot');
	    return function (c) {
	...
	var _hash_join = function (a, b) {
		a = hash_ify(a);
		b = hash_ify(b);
		Object.keys(b).forEach(function (x) {a[x] = b[x]});
	    return a;
	}
	...
	var _pre_defined = function (x) {
		var g = get_name(x);
		return (g in operators) || (g in adverbs)
		|| (g in built_ins) || (g in dicts);
	}
	...
	*/
	...
	var parse = function (s) {
		...
		for (var c = 0; c < s.length; c++) {
			...
			if (depth === 0) {
				if (character === '(') {
					...
					throw 'unparsable string; too many close parentheses';
					...
					var character_list = [];
					var mode = 'normal';
					var internal_character;
					c++;
					for (;;c++) {
						if (s.length <= c) {
							throw 'string not closed';
						}
						internal_character = s[c];
						if (mode === 'normal') {
							if (internal_character === character) {
								break;
							} else if (internal_character === '`') {
								mode = 'special_char';
							} else {
								character_list.push(internal_character);
							}
						} else if (mode === 'special_char') {
							if (internal_character in special_char_dict) {
								character_list.push(
									special_char_dict[internal_character]);
								mode = 'normal';
							} else {
								throw 'improper escape sequence';
							}
						} else {
							throw 'internal bug: string parsing '
							+ 'in interpreter is broken';
						}
					}
		...
		if (depth > 0) {
			throw 'unparsable string; not enough close parentheses';
		} else if (depth < 0) {
			throw 'internal bug: depth in parsing is broken';
		}
	...
	var exec_from_parse = function (l) {
		if (typeof l === 'string') {
			...
			throw 'could not execute; empty node in the program tree';
		} else if (l.length === 1) {
		...
		if (right_type === 'adverb') {
			...
			if (right_type === 'operator') {
			    throw 'could not execute; type mismatch (operator operator)';
	    	} else {
			    return function (x) {return left(x, right)};
		    }
