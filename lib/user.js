...
// @new
User.set_initial_data = function (uid, name, school, grade, class_number, email, avatar, year_of_initialization, callback) {
    var data = {
        profile: {
            name: name, 
            ...
            email: email,
            avatar: avatar,
            year_of_initialization: year_of_initialization
        },
        history: {
            sentence_logs: {}
        }
...
User.prototype.start_module = function (id) {
    ...
    if (!(id in this.data.history || this.is_mf())) {
        ...
        // and thus should not persist
        if (!is_spelling_mod_id(id)) {
...
User.prototype.create_module = function (mod_id) {
    ...
    // If module id is 0.25 or 0.5, we are in a module without history
    // and thus shoudl not persist
    if (this.uid != null && !is_spelling_mod_id(mod_id)) {
...
User.prototype.get_word_scores = function (callback) {
    Persist.get(['word_scores', this.uid], callback);
}

// {item: "arthropod", type: "word", correct: true, used_all_attempts: false, hints: 2, attempts: 2}
User.prototype.persist_word_scores = function (word_scores_update_list, callback) {
    console.log('paper airplane: persist word scores called');
    var uid = this.uid;
    var done = 0;
    for (var i = 0; i < word_scores_update_list.length; i++) {
        Persist.get(['word_scores', uid, word_scores_update_list[i].item], (function (item) {
            return function (x) {
                var data = x.val();
                var updated_data;
                var correct_increment = bool_to_num(item.correct);
                var used_all_attempts_increment = bool_to_num(item.used_all_attempts);
                if (data === null) {
                    updated_data = {
                        'item': item.item,
                        'type': item.type,
                        'correct': correct_increment,
                        'used_all_attempts': used_all_attempts_increment,
                        'hints': item.hints,
                        'total': 1
                    }
                } else {
                    updated_data = {
                        'item': item.item,
                        'type': item.type,
                        'correct': data.correct + correct_increment,
                        'used_all_attempts': data.used_all_attempts + used_all_attempts_increment,
                        'hints': data.hints + item.hints,
                        'total': data.total + 1
                    }
                }
                // add mastery
                updated_data.mastery = calculate_word_mastery(
                    updated_data.correct, updated_data.total, 5);
                console.log('paper airplane: persist word scores: updated_data =', updated_data);
                Persist.set(['word_scores', uid, item.item], updated_data);
                done++;
                if (done === word_scores_update_list.length) {
                    callback();
                }
            }
        })(word_scores_update_list[i]));
    }
}
...
User.prototype.get_spelling_tournament_module = function() {
    return 0.1;
};
...
User.prototype.update_question_metrics = function(incorrect, module_id) {
    ...
    console.log('hello module_id = ', module_id);
    
    // new version
    if (is_spelling_mod_id(module_id) || !module_id) {
        console.log("TODO short term solution to the missing mod in spelling_bee_trainer");
        return;
    }
    ...
    console.log("FEBRUARY BUG checkpoint 6");
    console.log("FEBRUARY BUG mod.metrics = ", mod.metrics);
    console.log("FEBRUARY BUG stringified mod.metrics = ", JSON.stringify(mod.metrics));
    // mod.metrics looks like it mostly is a list of lists e.g. [[10, 1, 2, 1, 1]]
