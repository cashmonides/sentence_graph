...
User.prototype.user_data_loaded = function(data, callback){
    try {
        ...
        var email = this.data.profile.email;
        console.log("DEBUG 5-18 email = ", email);
        if (email.endsWith(".mf")) {
            console.log("DEBUG 5-18 mf regime change successful = ");
...
User.prototype.start_module = function (id) {
    ...
    console.log("DEBUG 11-16 history in this module = ", history_here);
...
User.prototype.submodule_complete = function (module_id) {
    ...
    console.log("DEBUG 1-29 entering problem area");
    console.log("12-30 mod = ", mod);
    console.log("12-30 mod.progress before increment = ", mod.progress);
    mod.progress++;
    console.log("DEBUG 1-29 mod.progress after increment", mod.progress);
    ...
    console.log("DEBUG 11-16 entering rollover mod.progress, "
...
function (sentence_logs, sentence_finder, next_sentence, status) {
    var chapter = sentence_finder.chapter_n;
    
    var sentence_number = sentence_finder.question_n;
    console.log("sentence_logs, next_sentence, chapter, sentence_number, status =",
    sentence_logs, next_sentence, chapter, sentence_number, status);
    
    if (!sentence_logs) {
        sentence_logs = {};
    }
    
    if (!(chapter in sentence_logs)) {
        sentence_logs[chapter] = {};    
    }
    
    if (status in sentence_logs[chapter]) {
        sentence_logs[chapter][status].push(sentence_number);
    } else {
        sentence_logs[chapter][status] = [sentence_number];
    }
    
    if (next_sentence.chapter_n !== chapter) {
        sentence_logs.frontier_chapter = next_sentence.chapter_n;
        if (!(next_sentence.chapter_n in sentence_logs)) {
            sentence_logs[next_sentence.chapter_n] = {
                'frontier': next_sentence.question_n
            }
        }
        sentence_logs[chapter].frontier = null;
    } else {
        sentence_logs[chapter].frontier = next_sentence.question_n;
    }
