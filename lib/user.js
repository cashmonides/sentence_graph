...
User.prototype.user_data_loaded = function(data, callback){
    try {
        ...
        // console.log("BACKLOG user data:", this.data);
        
        if (this.is_mf()) {
            console.log("LOG change to mf regime");
...
User.prototype.submodule_complete = function (module_id) {
    debuglog("[user.submodule_complete] entering submodule_complete");
    // It is very important to check the user here.
    this.important_event_check_string();
    var self = this;
    var mod = this.data.history[module_id];
    debuglog("[user.submodule_complete] entering problem area");
    debuglog("[user.submodule_complete] mod = ", mod);
    debuglog("[user.submodule_complete] mod.progress before increment = ", mod.progress);
    mod.progress++;
    debuglog("[user.submodule_complete] mod.progress after increment", mod.progress);
    ...
    debuglog("[user.submodule_complete] entering rollover mod = ", mod);
    debuglog("[user.submodule_complete] entering rollover mod.progress, "
    + "ALL_MODULES[module_id].threshold = ", mod.progress,
    ALL_MODULES[module_id].threshold);
    debuglog("[user.submodule_complete] entering problem area, rollover");
    debuglog("[user.submodule_complete] rollover is", rollover);
    //a module has been completed (e.g. 5/5)
    var result = null;
    if (rollover) {
        debuglog("[user.submodule_complete] rollover if block entered");
        result = function () {
            var mod = self.data.history[module_id];
            mod.iteration++;
            mod.progress = 0;
            mod.in_progress = false;
            debuglog("[user.submodule_complete] inside rollover mod = ", mod);
            debuglog("[user.submodule_complete] inside rollover mod.progress = ", mod.progress);
            debuglog("[user.submodule_complete] in progress", mod.in_progress);
            ...
            if (self.quiz.advance_improve_status === "improving") {
                debuglog("[user.submodule_complete] return to homebase triggered");
    ...
    debuglog("[user.submodule_complete] result = ", result);
    return result;
};
...
User.prototype.update_question_metrics = function(incorrect, module_id) {
    debuglog("[user.update_question_metrics] entering user.update_question_metrics");
    debuglog("[user.update_question_metrics] incorrect = ", incorrect);
    debuglog("[user.update_question_metrics] module_id = ", module_id);
    
    // This is called when going into a module.
    var mod = this.data.history[module_id];
    debuglog("[user.update_question_metrics] mod from history = ", mod);
...
User.prototype.init_metrics = function (mod) {
    debuglog("[user.update_question_metrics] entering init_metrics");
    if (!mod.metrics) {
        debuglog("[user.update_question_metrics] !mod.metrics so initializing an empty map");
        mod.metrics = {};    
    }
    
    //just did 1st question on a new iteration of the module
    if (!(mod.iteration in mod.metrics)) {
        debuglog("[user.update_question_metrics] !mod.iteration so initializing an empty map");
        mod.metrics[mod.iteration] = {};            //creates key and value
    }
    debuglog("[user.update_question_metrics] leaving init_metrics");
}
...
function (sentence_logs, path, status, mode, callback) {
    console.log("sentence_logs, path, status =",
...
User.prototype.get_accuracy_list = function (module_id) {
    ...
    debuglog("[user.get_accuracy_list] entering user.get_accuracy_list")
    debuglog("[user.get_accuracy_list] module_id = ", module_id);
    ...
    debuglog("[user.get_accuracy_list] metrics = ", metrics);
    ...
    debuglog("[user.get_accuracy_list] accuracy_list =", accuracy_list);
    return accuracy_list;
}


User.prototype.get_max_accuracy = function (module_id) {
    debuglog("[user.get_max_accuracy] module_id = ", module_id);
