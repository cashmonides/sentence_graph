...
User.prototype.get_initial_spelling_bee_level = function() {
    console.log("BEEHACK123 this.data.spelling_level = ", this.data.spelling_level);
    return this.data.spelling_level;
};


User.prototype.persist_spelling_bee_level = function (level) {
    this.persist(["spelling_level"], level, function (x) {
        console.log('empty callback triggered', x);
    });
}
...
User.prototype.update_question_metrics = function(incorrect, module_id) {
    console.log("[user.update_question_metrics] entering user.update_question_metrics");
    console.log("[user.update_question_metrics] incorrect = ", incorrect);
    console.log("[user.update_question_metrics] module_id = ", module_id);
    
    // This is called when going into a module.
    var mod = this.data.history[module_id];
    console.log("[user.update_question_metrics] mod from history = ", mod);
...
User.prototype.init_metrics = function (mod) {
    console.log("[user.update_question_metrics] entering init_metrics");
    if (!mod.metrics) {
        console.log("[user.update_question_metrics] !mod.metrics so initializing an empty map");
        mod.metrics = {};    
    }
    
    //just did 1st question on a new iteration of the module
    if (!(mod.iteration in mod.metrics)) {
        console.log("[user.update_question_metrics] !mod.iteration so initializing an empty map");
        mod.metrics[mod.iteration] = {};            //creates key and value
    }
    console.log("[user.update_question_metrics] leaving init_metrics");
}
...
User.prototype.get_accuracy_list = function (module_id) {
    ...
    console.log("[user.get_accuracy_list] entering user.get_accuracy_list")
    console.log("[user.get_accuracy_list] module_id = ", module_id);
    ...
    console.log("[user.get_accuracy_list] metrics = ", metrics);
    ...
    console.log("[user.get_accuracy_list] accuracy_list =", accuracy_list);
    return accuracy_list;
}


User.prototype.get_max_accuracy = function (module_id) {
    console.log("[user.get_max_accuracy] module_id = ", module_id);
