...
User.prototype.submodule_check_string = function () {
    var result = this.check_for_string();
    switch (result) {
        case "not logged in":
        ...
        case "OK":
            break;
        default:
            throw new Error('Unforseen result from this.check_for_string().');
    }
    return result;
}
...
User.prototype.submodule_complete = function (module_id) {
    var state = this.submodule_check_string();
    if (state !== 'OK') {
        if (state === 'two computers') {
            this.set_login_status(this.login_status);
        }
        return null;
    };
...
User.prototype.set_login_status = function (login_status) {
    this.persist(["profile", "login_status"], login_status);
}

User.prototype.get_persist_login_status = function () {
    Persist.get_path(["users", this.uid, "login_status"]);
}

User.prototype.check_for_string = function () {
    var current_value = this.get_persist_login_status()
    if (current_value === null) {
        return 'not logged in';
    } else if (current_value !== this.login_status) {
        return 'two computers';
    } else {
        return 'OK';
    }
}
