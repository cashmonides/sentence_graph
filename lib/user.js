...
User.prototype.submodule_complete = function (module_id) {
    console.log("BREITBART entering submodule_complete");
    // It is very important to check the user here.
    this.important_event_check_string();
    var self = this;
    var mod = this.data.history[module_id];
    console.log("BREITBART entering problem area");
    console.log("BREITBART mod = ", mod);
    console.log("BREITBART mod.progress before increment = ", mod.progress);
    mod.progress++;
    console.log("BREITBART mod.progress after increment", mod.progress);
    ...
    console.log("BREITBART entering rollover mod = ", mod);
    console.log("BREITBART entering rollover mod.progress, "
    + "ALL_MODULES[module_id].threshold = ", mod.progress,
    ALL_MODULES[module_id].threshold);
    console.log("BREITBART entering problem area, rollover");
    console.log("BREITBART rollover is", rollover);
    //a module has been completed (e.g. 5/5)
    var result = null;
    if (rollover) {
        console.log("BREITBART rollover if block entered");
        result = function () {
            var mod = self.data.history[module_id];
            mod.iteration++;
            mod.progress = 0;
            mod.in_progress = false;
            console.log("BREITBART inside rollover mod = ", mod);
            console.log("BREITBART inside rollover mod.progress = ", mod.progress);
            console.log("BREITBART in progress", mod.in_progress);
    ...
    console.log("BREITBART in submodule_complete result = ", result);
    return result;
};
...
User.prototype.update_question_metrics = function(incorrect, module_id) {
    console.log("BREITBART entering user.update_question_metrics");
    console.log("BREITBART incorrect = ", incorrect);
    console.log("BREITBART module_id = ", module_id);
    
    // This is called when going into a module.
    var mod = this.data.history[module_id];
    console.log("BREITBART mod from history = ", mod);
...
User.prototype.init_metrics = function (mod) {
    console.log("BREITBART entering init_metrics");
    if (!mod.metrics) {
        console.log("BREITBART !mod.metrics so initializing an empty map");
        mod.metrics = {};    
    }
    
    //just did 1st question on a new iteration of the module
    if (!(mod.iteration in mod.metrics)) {
        console.log("BREITBART !mod.iteration so initializing an empty map");
        mod.metrics[mod.iteration] = {};            //creates key and value
    }
    console.log("BREITBART leaving init_metrics");
}
...
User.prototype.get_accuracy_list = function (module_id) {
    ...
    console.log("BREITBART entering user.get_accuracy_list")
    console.log("BREITBART module_id = ", module_id);
    ...
    console.log("BREITBART in get_accuracy_list metrics = ", metrics);
    ...
    console.log('BREITBART LOG: accuracy_list =', accuracy_list);
