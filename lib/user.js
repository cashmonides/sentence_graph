...
User.prototype.classify_module = function (mod_id) {
    console.log("DEBUGGING 11-20 classify_module entered");
...
User.prototype.get_max_accuracy = function (module_id) {
    //we need a metrics object
    var metrics = this.data.history[module_id].metrics;
    //we get the accuracy on all of the iterations of the metrics objecgt
    var accuracy_list = this.get_accuracy_list(metrics);
    return Math.max.apply(null, accuracy_list);
    
    
    // Object.keys(mod_metrics).map(function (key) {
    // return this.get_accuracy2 (mod_metrics[key]);
    // });
}
