...
User.prototype.get_personal_data = function (quiz) {
    ...
    var magic_number = types_of_data.length;
    for (var i = 0; i < magic_number; i++) {
        var x = types_of_data[i];
        Persist.get(["users", this.uid, 'profile', x], (function (i) {
            return function (data) {
                console.log(i, quiz.time_data);
                quiz.time_data[i + 1] = data;
                quiz.found++;
                if (quiz.found === magic_number) {
                    quiz.found = 0;
