...
Sentence.prototype.has_region = function (indices) {
    for (var i in this.regions) {
        if (this.regions[i].equals_list(indices)) {
            return true;
        }
...
Sentence.prototype.get_sub_regions = function (region) {
    var super_list = region.get_indices();
    var results = [];
    for (var i in this.regions) {
        if (is_sub_list(super_list, this.regions[i].get_indices()))  {
...
Sentence.prototype.get_all_tag_types = function(region_filter, tag_filter){
    ...
    regions.forEach(function(r){
        ...
        // console.log('region, tags =', r, tags);
        // old code
        // r.get_tag_types().forEach(function(t){
        tags.forEach(function(t){
            // new rejected code
            // tag_region_combos.push({'region': r, 'tag': t});
            // console.log('tag added to all tags =', t);
...
Sentence.deserialize = function(data){
    ...
    console.log("LOG: # of sentences loaded:", unique_ids.length);
