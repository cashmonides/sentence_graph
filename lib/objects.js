...
Region.prototype.get_indices = function () {
    ...
    var list_of_indices = array_from(set_of_indices);
    //a-b retur s
...
Region.prototype.remove_duplicate_tags = function () {
    var tag_types_already_seen = [];
    var new_tags = [];
    this.tags.forEach(function (x) {
        if (tag_types_already_seen.indexOf(x.get_tag_type()) === -1) {
            tag_types_already_seen.push(x.get_tag_type());
        }
    })
    this.tags = new_tags;
};
