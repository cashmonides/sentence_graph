...
var alphabetize_dict = function (d) {
    return quick_sort(key_value_pairs(d), function (x, y) {return x[0] < y[0]});
}
...
// The main reason this is a util is because it looks unclear.
...
// todo design issue; should this really go in utils?
...
var create_cheat_sheet_table = function (o, table_id, left_class, right_class,
left_f, right_f, list_of_pairs) {
    var e = make({'tag': 'table', 'id': table_id,
    'style': {'display': 'block'}}, o);
    var left_make = {'tag': 'td'};
    if (left_class !== null) {left_make['class'] = left_class};
    var right_make = {'tag': 'td'};
    if (right_class !== null) {right_make['class'] = right_class};
    for (var i = 0; i < list_of_pairs.length; i++) {
