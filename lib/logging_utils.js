...
var Buglogger = function(gState, klass, switch_status) {
    ...
    if ((development_status === 'development') && (switch_status === 'bug_logging')) {
        // we've hit a big bug
        // so we trigger some heavy duty alert
        // ideally we add some info to it but for now just an alert
        alert("BUG CAUGHT BY BUGLOGGER");
        // and continue and lopg the error in the console for good measure
