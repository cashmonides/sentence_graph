...
var sort_by_prop = function (s) {
    return function (x, y) {return (x[s] < y[s]) ? -1 : ((x[s] > y[s]) ? 1 : 0)};
}

var sort_by_value = function (d) {
    return function (x, y) {return (d[x] < d[y]) ? -1 : ((d[x] > d[y]) ? 1 : 0)};
}

var sort_by_func = function (f) {
    return function (x, y) {return (f(x) < f(y)) ? -1 : ((f(x) < f(y)) ? 1 : 0)};
}
