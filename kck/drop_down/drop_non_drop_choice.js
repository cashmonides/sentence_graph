...
// Tries to be logical.
var choose_drops_and_non_drops = function (list_of_role_strings, level) {
    var drop_non_drop_settings = get_drop_non_drop_settings(level);
    ...
    if (min > max) {
        throw 'Very serious error! min (' + min + ') > max (' + max + ')';
    }
    var choices;
